...
export const LoginPage = () => {
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-surface px-4 py-8 sm:px-6 lg:px-8">
      <Card className="w-full max-w-xl p-6 sm:p-7">
        ...
        <h1 className="mt-2.5 text-3xl font-semibold tracking-tight text-slate-950">Admin Sign In</h1>
        ...
        <form className="mt-6 space-y-3.5" onSubmit={handleSubmit}>
          <label className="block text-sm font-medium text-slate-700">
            ...
            <input
              ...
              className="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-900 outline-none transition focus:border-slate-400"
              autoComplete="email"
              required
            />
          </label>
          ...
          {error ? <p className="rounded-xl border border-red-200 bg-red-50 px-3.5 py-2.5 text-sm text-red-700">{error}</p> : null}
